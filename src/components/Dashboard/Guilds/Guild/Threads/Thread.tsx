...
import { ThreadsProps } from ".";
import { GuildPayload, GuildThreadsPayload } from "../../../../../types";
...
export default function Thread({ thread, setThreads, threads, guild, setGuild, setModifications }: Props) {
    const l = useLanguage();
    ...
    const handleRemoveThread = async () => {
        ...
        const filtredThreads = threads.filter((threada) => threada.id !== thread.id);

        const delete_threads_ids = threads.filter((threada) => threada.id === thread.id).map(() => {
            return {
                id: thread.id,
