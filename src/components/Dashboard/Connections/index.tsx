                ...
                <div className="grid grid-cols-3 gap-3 w-full">
                    {connections ? (
                        connections.filter((connection) => connection.name.toLowerCase().includes(searchQuery.toLowerCase()) || connection.name.includes(searchQuery)).map((connection) => (
                            <button onClick={() => openModal(connection.name)} key={connection.name} className="flex items-center gap-2 p-3 rounded-lg bg-neutral-800 hover:bg-neutral-700 transition">
                    ...
                    <Modal classNames={{
                        ...
                        <ModalContent className="bg-neutral-800 text-white">
                            <ModalHeader className="flex flex-col gap-1 bg-neutral-800">Editar conexão {connectionId}</ModalHeader>
                            <ModalBody>
                                <ConnectionComponent id={connectionId} />
                            </ModalBody>
                        </ModalContent>
                    </Modal>
                </div>
