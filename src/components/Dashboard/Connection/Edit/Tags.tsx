...
export default function ConnectionEditTag({
	editedConnection,
	setEditedConnection,
	setModifications,
	connection,
	...
	const addNewTag = (event: ChangeEvent<HTMLInputElement>) => {
		...
		if (value.includes(" ")) {
			const newWords = value
				.split(" ")
				.filter((w) => w.length > 0 && !editedConnection.tags?.includes(w));
			...
			if (newWords.length > 0) {
				...
				const areTagsEqual =
					connection.tags.length === updatedTags.length &&
	...
	return (
		<div className="flex gap-1 flex-col">
			...
			<div
				className="flex flex-wrap gap-2 p-3 rounded-lg bg-neutral-900/50 
            min-h-14 min-w-full"
			>
				<AnimatePresence>
