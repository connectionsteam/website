...
import { ConnectionMetrics, ConnectionPayload } from "../../../../types";
...
export default function PromotedMetricsComponent({ connection }: Props) {
    ...
    const lastViews = metrics?.views.filter((view) => new Date(view).getTime() > Date.now() - seven_days);

    const lastServers = metrics?.servers.filter((view) => new Date(view).getTime() > Date.now() - seven_days);

    const views = metrics?.views
        && metrics.views.length > 1
                ...
                <div className="flex flex-col gap-3">
                    <div>
                        <div className="flex items-center gap-2">
                            <h1 className="font-bold text-xl">{l.dashboard.connections.metrics.title}</h1>
                    ...
                    <div className="flex flex-col gap-2">
                        <div className="flex flex-col gap-2">
                            <h2 className="font-bold text-lg">{l.dashboard.connections.metrics.visualizations.title}</h2>
                                ...
                                {lastViews?.length === 0 ? (
                                    ...
                                    <>
                                        <div>
                                            ...
                                            {views} {l.dashboard.connections.metrics.visualizations.total}
                                        </div>
                                        <div>
                                            <span className="text-[#0DF228] font-bold text-lg">+{lastViews?.length ?? 0} {" "}</span>
                                            {views} {l.dashboard.connections.metrics.visualizations.lastseven}
                                        </div>
                                    </>
                                )}
                            </div>
                        ...
                        <div className="flex flex-col gap-2">
                            <h2 className="font-bold text-lg">{l.dashboard.connections.metrics.servers.title}</h2>
                            <div className="bg-neutral-900/50 rounded-lg w-full p-4 flex flex-col gap-2">
                                <div>
                                    {metrics?.servers.length === 0 ? (
                                        ...
                                        <>
                                            ...
                                            <div>
                                                <span className="text-[#0DF228] font-bold text-lg">+{lastServers?.length ?? 0} {" "}</span>
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="bg-neutral-900/50 rounded-lg w-full p-3 flex flex-col gap-2 items-center justify-center">
                                <div className="text-lg">👀 {l.dashboard.connections.metrics.feedbacks.commingSoon}</div>
                            </div>
                        </div>
                        <div className="flex flex-col gap-2 w-full">
                            <h2 className="font-bold text-lg">{l.dashboard.connections.metrics.graphic}</h2>
                            <ChartComponent views={metrics?.views} servers={metrics?.servers} feedbacks={metrics?.feedbacks} />
                        </div>
                    </div>
                </div>
            ) : (
