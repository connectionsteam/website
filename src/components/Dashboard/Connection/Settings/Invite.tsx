...
interface Props {
	...
	setEditedConnection: (e: Partial<ConnectionPayload>) => void;
...
export default function ConnectionPrivateInvite({
	...
	const updateInvite = async () => {
		...
		const {
			data: { invite },
		} = await api.put(`/connections/${connection.name}/invite`);
	...
	const handleCopyLink = () => {
		...
		navigator.clipboard.writeText(
			`${new URL(location.href).origin}/invite/${connection.name}/${connection.hashInvite}`,
		);
			...
			<div className="flex flex-col gap-1">
				<div className="flex gap-1">
					...
					<div className="font-bold text-lg">
						{l.dashboard.connections.connection.private}
					</div>
				</div>
				...
				<div className="flex gap-2">
                    ...
                    w-full px-3 bg-neutral-900/50 rounded-lg"
						>
							...
							<input
								disabled
								value={`${new URL(location.href).origin}/invite/${connection.name}/${
									showHash
										? connection.hashInvite
										: connection.hashInvite?.replaceAll(
												connection.hashInvite,
