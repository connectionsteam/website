...
import { ConnectionBody, GuildPayload } from "../../../types";
...
export default function JoinPrivateConnectionModal({ isOpen, onOpenChange, code: { name, code }, guilds }: Props) {
    const l = useLanguage();
                            ...
                            {guilds ? (
                                <Dropdown className="bg-neutral-800 text-white w-full max-h-56 overflow-x-auto">
                                    ...
                                    <DropdownMenu className="w-full flex flex-col gap-1" aria-label="guilds">
                                        {guilds
                                            .filter((guild) => !guild.connections.map((c) => c.name).includes(name))
                                            .map((guild) => (
                                                <DropdownItem
