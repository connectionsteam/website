...
export default function DefaultPremiumButton({
	disabled,
	text,
	className,
	onClick,
	...
	return (
		<div
			className={`p-0.5 rounded-lg bg-gradient-to-r ${pink ? "from-pink-500 to-rose-700" : "from-amber-400 to-orange-500"} w-full`}
		>
			{link ? (
				<Link
					href={href || ""}
				...
				>
					{loading ? (
						...
						<HiSparkles
							className="fill-yellow-500 group-hover:fill-white transition group-disabled:group-hover:fill-yellow-500"
							size={23}
						/>
					)}
					<span>{text}</span>
