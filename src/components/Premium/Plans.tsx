			...
			<div className="w-full flex items-center gap-3 flex-col">
				<div className="flex gap-1 flex-col items-center w-full">
					<div className="w-full flex items-center text-center">
						<button
							className="flex gap-2 items-center transition hover:bg-neutral-700 bg-neutral-800 p-2 rounded-lg group"
							onClick={() => setMorePlans(false)}
						>
							<LuArrowLeft size={20} />
							<span>{l.dashboard.guilds.connections.infos.back}</span>
						</button>
						<span className="font-bold text-2xl w-full text-center mr-[88px]">
							Connections
						</span>
					</div>
					<h1 className="text-6xl p-2 font-bold bg-clip-text text-transparent bg-gradient-to-r from-fuchsia-500 to-indigo-500">
						Premium
					</h1>
				</div>
				<TextGenerateEffect
					className="text-center flex items-center text-lg text-neutral-300"
					words={l.plans.pageDescription}
				/>
			</div>
				...
				{plans.map((plan, index) => {
					...
					return (
						<BackgroundGradient
							...
							animate={{ x: 0, opacity: 1 }}
							transition={{ delay: index * 0.2 }}
							className="h-full tablet:w-full"
							key={index}
						>
							{PlanComponent}
						</BackgroundGradient>
