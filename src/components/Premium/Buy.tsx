...
export default function PopUpBuy({ onClose }: { onClose: () => void }) {
	const l = useLanguage();

	return (
		<ModalContent className="bg-neutral-800 text-white">
			<ModalHeader className="pb-1">
				<div>{l.plans.buypopUp.title}</div>
			</ModalHeader>
			<ModalBody className="flex gap-2">
				<div className="bg-yellow-500/10 rounded-lg p-2 flex gap-2 items-center mobile:flex-col">
					...
					<div className="font-bold">{l.plans.buypopUp.warning}</div>
				</div>
				<div className="flex flex-col gap-2">
					<div className="text-lg">
						{l.plans.buypopUp.fristimageDescription}
						<a
							href={discordLink}
							target="_blank"
							className="ml-1.5 text-blue-500 transition hover:text-blue-400 underline" rel="noreferrer"
						>
							{l.plans.buypopUp.discordServer}
						</a>
						{l.plans.buypopUp.firstimageDescriptiontwo}
					</div>
					<Image
				...
				<div className="flex flex-col gap-2">
					<div className="text-lg">
						{l.plans.buypopUp.secondImageDescription}
					</div>
					<Image
