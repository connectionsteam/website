...
export default function SearchConnection({ actualConnections, setFilters, filters, query, setQuery, setSubmited, queryInput }: Props) {
    const l = useLanguage();
    ...
    const debouncedFetchConnections = useCallback(debounce(async (query: string) => {
        setLoading(true);
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (panelRef && panelRef.current && !panelRef.current.contains(event.target as Node)) {
                setPanel(false);
            }
        }
    ...
    return (
        <div>
            <div className={`bg-neutral-800 rounded-lg border-2 transition 
                ...
                <DefaultInput
                    ...
                    onChange={(e) => {
                        ...
                        setFilters({ ...filters, query: e.target.value.toLowerCase(), search: false });
                    }}
                    type="string"
                    label=""
                />
                            ...
                            <div className="flex w-full flex-col gap-4 h-full">
                                <div className="flex w-full">
                                    <AnimatePresence mode="wait">
                                        <motion.div className={`grid gap-3 w-full 
                                            ${connections.length <= 0 
                                            ? "grid-cols-1 tablet:grid-cols-1" 
                                            : "grid-cols-2 mobile:grid-cols-1"}`}>
                                            {!loading ? connections.length <= 0 ? (
                                                <div className="w-full flex py-6 font-bold 
                                                text-center items-center justify-center">
                                                    <span>{l.connection.noConnections}</span>
                                                </div>
                                            ) : connections.map((connection, index) => (
                                                <ConnectionPageCard connection={connection} key={index} />
                                            )) : Array.from({ length: 3 }, (_, index) => (
                                                <div className="p-3 rounded-lg bg-neutral-800 
                                                items-center flex transition" key={index}>
                                                    <div className="flex gap-3">
                                                        <div className="w-12 h-12 rounded-full 
                                                        bg-neutral-700 animate-pulse" />
                                                        <div className="flex flex-col gap-2">
                                                            <div className="w-32 rounded-full h-4 
                                                            bg-neutral-700 animate-pulse"></div>
                                                            <div className="rounded-lg bg-neutral-700
                                                            p-2.5 w-10 animate-pulse"></div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ))}
