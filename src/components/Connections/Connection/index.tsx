...
const ConnectionsPageCard = forwardRef<HTMLDivElement, Props>(
	({ connection, index, layout, connections = [], guilds }, ref) => {
		...
		return (
			<motion.div {...animation} className="w-full relative" ref={ref}>
				{"promotingSince" in connection && (
					<div className="absolute px-2 font-semibold -top-3 flex rounded-full bg-rose-600 left-3">
				...
				<div
                    ...
                    ${"promotingSince" in connection ? "border-2 border-rose-600" : ""}`}
				>
					<Link
                ...
                `}
					>
                        ...
                        `}
						>
							<span className="font-bold text-xl mobile:text-lg">
								{connection.name}
							</span>
							...
							<div
								className={`text-neutral-300 px-2 bg-neutral-700 
